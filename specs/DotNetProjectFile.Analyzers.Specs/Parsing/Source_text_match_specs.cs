...
public class Matches
{
    ...
    {
        var source = new SourceSpan(Source.Text("Hello, World!"), new(start, 3));
    ...
    {
        var source = new SourceSpan(Source.Text("Hello, World!"), new(start, 13 - start));
...
public class Does_not_match
{
    ...
    public void not_matching_char()
        => new SourceSpan(Source.Text("a"), new(0, 1)).StartsWith('A').Should().BeNull();

    [Test]
    public void char_on_empty_source_text()
        => new SourceSpan(Source.Text("a"), new(0, 0)).StartsWith('a').Should().BeNull();

    [Test]
    public void not_matching_string()
        => new SourceSpan(Source.Text("abc"), new(0, 3)).StartsWith("ABc").Should().BeNull();

    [Test]
    public void string_longer_then_source_text()
        => new SourceSpan(Source.Text("abc"), new(0, 2)).StartsWith("ABC").Should().BeNull();

    [Test]
    public void regex()
        => new SourceSpan(Source.Text("abc"), new(0, 2)).Matches("[0-9]+").Should().BeNull();
}
...
public class Regexes
{
    ...
    {
        var source = new SourceSpan(Source.Text("abcde"), new(0, 5));
